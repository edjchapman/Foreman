...
environment variable (secrets generated by `random_password`), and both
domains — the generated `*.up.railway.app` one plus the custom
`foreman.edwardchapman.co.uk` (create the CNAME + verification TXT from the
`custom_domain_dns` output in the DNS zone, CNAME kept DNS-only so Railway
can issue the cert). Rationale and the full deploy story: [ADR 0005](../../../docs/adr/0005-deployment-platform.md)
