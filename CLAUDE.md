...
- `make deploy VERSION=<x.y.z>` — deploy to Railway (`scripts/railway-deploy.sh`: GHCR pre-flight rejects unpublished tags, then pins the semver image tag per service via the GraphQL API; web deploys first and gates worker/beat behind pre-deploy `migrate` + `/readyz`, then worker/beat are polled to SUCCESS too). Runs automatically from the `deploy` job in `release-please.yml` on each release; needs `RAILWAY_TOKEN` (secret) + `RAILWAY_{ENVIRONMENT,WEB_SERVICE,WORKER_SERVICE,BEAT_SERVICE}_ID` (repo variables). Topology/provisioning/rollback: `docs/deploy.md`; rationale: ADR 0005.
- Platform provisioning is Terraform (`deploy/terraform/` — project, 5 services, secrets, env vars, domain; local git-ignored state). `terraform destroy`/`apply` is the demo's off/on switch. Three settings the provider can't express (worker/beat start commands, web pre-deploy + healthcheck) are applied by `make configure` (`scripts/railway-configure.sh`, idempotent) after `apply`; the only manual step is the CD project token (`terraform output manual_steps`). `make tf-check` (fmt + validate) gates the module in CI; `deploy.yml` (workflow_dispatch) covers rollback / post-rebuild re-pinning.
