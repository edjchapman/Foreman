...
- Platform provisioning is Terraform (`deploy/terraform/` — project, 5 services, secrets, env vars, domain; local git-ignored state). `terraform destroy`/`apply` is the demo's off/on switch. Three settings the provider can't express (worker/beat start commands, web pre-deploy + healthcheck) are one-time dashboard steps — `terraform output manual_steps`.
